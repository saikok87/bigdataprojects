...
public class HBaseM7BulkLoadMapper extends Mapper<LongWritable, Text, ImmutableBytesWritable, Put> implements Constants{
	private static final Logger logger = Logger.getLogger(HBaseM7BulkLoadMapper.class);
	...
	protected void map(LongWritable key, Text value,
		...
		if(strArrCols.length == fields.length){
			...
			hKey.set(Bytes.toBytes(rowKeyIndex));
			
			if(fields[0] != null && !fields[0].isEmpty() && !fields[0].trim().equalsIgnoreCase("") && !("\\N").equalsIgnoreCase(fields[0])) {
				
				//Put put = new Put(Bytes.toBytes(fields[0]));
				Put put = new Put(Bytes.toBytes(rowKeyIndex));
				put.add(COL_FAMILY, strArrCols[0].getBytes(), fields[0].trim().getBytes());
